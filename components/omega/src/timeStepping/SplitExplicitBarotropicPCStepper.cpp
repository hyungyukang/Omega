...
void SplitExplicitBarotropicPCStepper::doBarotropicVelocityUpdate(
   ...
   if (HaloWidth < 3)
      LOG_CRITICAL("Split-explicit barotropic subcycling needs a Decomp "
                   "HaloWidth of at least 3, got {}",
                   HaloWidth);

   const I4 VelPredEdgeRange   = Mesh->NEdgesHaloH(HaloWidth - 1);
   const I4 PressPredCellRange = Mesh->NCellsHaloH(HaloWidth - 2);
   const I4 VelCorrEdgeRange   = Mesh->NEdgesHaloH(HaloWidth - 2);
   const I4 PressCorrCellRange = Mesh->NCellsHaloH(HaloWidth - 3);
   ...
   for (I4 Subcycle = 0; Subcycle < NBtrSubcycles; ++Subcycle) {
      ...
      parallelFor(
          "btrVelocityPredictor", {VelPredEdgeRange},
      ...
      parallelFor(
          "btrVelocityCorrector", {VelCorrEdgeRange},
          KOKKOS_LAMBDA(I4 IEdge) {
             const I4 KMin = MinLayerEdgeBot(IEdge);
             if (MaxLayerEdgeTop(IEdge) < KMin) {
