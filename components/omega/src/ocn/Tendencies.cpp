...
void Tendencies::readConfig(Config *OmegaConfig ///< [in] Omega config
   ...
   void Tendencies::defineFields() {
      ...
      auto PseudoThicknessTendField = Field::create(
          ...
          "cell_thickness_tendency", -9.99E+10, 9.99E+10, -9.99E+30, NDims,
          DimNamesThickness);
      NDims = 3;
   ...
   Tendencies::Tendencies(
       const std::string &Name_, ///< [in] Name for tendencies
       ...
       : Mesh(Mesh), VCoord(VCoord), VAdv(VAdv),
         ...
         CustomVelocityTend(InCustomVelocityTend), EqState(EqState),
         PGrad(PGrad), VMix(VMix) {

      // Tendency arrays
      PseudoThicknessTend = Array2DReal("PseudoThicknessTend", Mesh->NCellsSize,
                                        VCoord->NVertLayers);
      NormalVelocityTend  = Array2DReal("NormalVelocityTend", Mesh->NEdgesSize,
                                        VCoord->NVertLayers);
             ...
             KOKKOS_LAMBDA(int ICell, const TeamMember &Team) {
                ...
                parallelForInner(
                    Team, KRange, INNER_LAMBDA(int KChunk) {
                       LocThicknessFluxDiv(LocPseudoThicknessTend, ICell,
                                           KChunk, ThickFluxEdge,
                                           NormalVelEdge);
                    });
             });
   ...
   void Tendencies::computeVelocityTendenciesOnly(
      ...
      if (CustomVelocityTend) {
         ...
         CustomVelocityTend(LocNormalVelocityTend, State, AuxState,
                            ThickTimeLevel, VelTimeLevel, Time);
   ...
   void Tendencies::setSurfaceTracerFlux(const Array2DReal &Flux) {
      OMEGA_REQUIRE(
          Flux.extent(0) == SurfaceTracerFlux.extent(0),
          "Tendencies::setSurfaceTracerFlux: tracer dimension mismatch");
      OMEGA_REQUIRE(
          Flux.extent(1) == SurfaceTracerFlux.extent(1),
   ...
   void Tendencies::computeStageVerticalMixing(
      ...
      if (Tracers::getIndex(TempIdx, "Temperature") != 0 ||
         ...
         LOG_WARN(
             "Tendencies::computeStageVerticalMixing: Temperature/Salinity "
      ...
      Array2DReal PressureMidDbar("KPP-PressureMidDbar", NCellsAll,
                                  NVertLayers);
      parallelFor(
      ...
      EqState->computeBruntVaisalaFreqSq(ConservTemp, AbsSalinity,
                                         PressureMidDbar, EqState->SpecVol);

      Array2DReal PotentialDensity("KPP-PotentialDensity", NCellsAll,
                                   NVertLayers);
