...
int testEosLinear() {
   ...
   parallelReduce(
       "CheckSpecVolMatrix-linear", {Mesh->NCellsAll, VCoord->NVertLayers},
...
int testEosLinearDisplaced() {
   ...
   parallelReduce(
       "CheckSpecVolDispMatrix-linear", {Mesh->NCellsAll, VCoord->NVertLayers},
       KOKKOS_LAMBDA(int i, int j, int &localCount) {
...
int testEosTeos10() {
   ...
   Array2DReal SpecVol = TestEos->SpecVol;
   parallelReduce(
       "CheckSpecVolMatrix-Teos", {Mesh->NCellsAll, VCoord->NVertLayers},
       KOKKOS_LAMBDA(int i, int j, int &localCount) {
          if (!isApprox(SpecVol(i, j), TeosSVExpValue, RTol)) {
             localCount++;
          }
       },
       numMismatches);
...
int testEosTeos10Displaced() {
   ...
   Array2DReal SpecVolDisplaced = TestEos->SpecVolDisplaced;
   parallelReduce(
       "CheckSpecVolDispMatrix-Teos", {Mesh->NCellsAll, VCoord->NVertLayers},
       KOKKOS_LAMBDA(int i, int j, int &localCount) {
          if (!isApprox(SpecVolDisplaced(i, j), TeosSVExpValue, RTol)) {
             localCount++;
          }
       },
       numMismatches);
