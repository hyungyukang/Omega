...
void testEosLinear() {
   ...
   parallelReduce(
       "CheckSpecVolMatrix-linear", {NCellsAll, NVertLayers},
       KOKKOS_LAMBDA(int I, int K, int &LocalCount) {
          if (!isApprox(SpecVol(I, K), LinearExpValue, RTol))
             ++LocalCount;
       },
       NumMismatches);
...
void testEosLinearDisplaced() {
   ...
   /// Check all array values against expected value
   int NumMismatches = 0;
   OMEGA_SCOPE(SpecVolDisplaced, TestEos->SpecVolDisplaced);
   parallelReduce(
       "CheckSpecVolDispMatrix-linear", {NCellsAll, NVertLayers},
       KOKKOS_LAMBDA(int I, int K, int &LocalCount) {
          if (!isApprox(SpecVolDisplaced(I, K), LinearExpValue, RTol))
             LocalCount++;
       },
       NumMismatches);
...
void testEosTeos10() {
   ...
   OMEGA_SCOPE(SpecVol, TestEos->SpecVol);
   parallelReduce(
       "CheckSpecVolMatrix-Teos", {NCellsAll, NVertLayers},
       KOKKOS_LAMBDA(int I, int K, int &LocalCount) {
          if (!isApprox(SpecVol(I, K), TeosSVExpValue, RTol)) {
             LocalCount++;
          }
       },
       NumMismatches);
...
void testEosTeos10Displaced() {
   ...
   OMEGA_SCOPE(SpecVolDisplaced, TestEos->SpecVolDisplaced);
   parallelReduce(
       "CheckSpecVolDispMatrix-Teos", {NCellsAll, NVertLayers},
       KOKKOS_LAMBDA(int I, int K, int &LocalCount) {
          if (!isApprox(SpecVolDisplaced(I, K), TeosSVExpValue, RTol)) {
             LocalCount++;
          }
       },
       NumMismatches);
