...
TangentAuxVars::TangentAuxVars(const std::string &AuxStateSuffix,
    ...
    : TangentVelEdge("TangentVelEdge" + AuxStateSuffix, Mesh->NEdgesSize,
                     VCoord->NVertLayers),
...
void TangentAuxVars::registerFields(const std::string &AuxGroupName,
   ...
   auto TangentVelEdgeField =
       Field::create(TangentVelEdge.label(), // field name
   ...
   FieldGroup::addFieldToGroup(TangentVelEdge.label(), AuxGroupName);

   // Attach data to fields
   TangentVelEdgeField->attachData<Array2DReal>(TangentVelEdge);
}

void TangentAuxVars::unregisterFields() const {
   Field::destroy(TangentVelEdge.label());
}
